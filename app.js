...
function render(){
 ...
 const base=data.papers.filter(p=>(topic==='all'||p.topics.includes(topic))&&(status==='all'||p.status===status)&&(!days||Date.now()-Date.parse(p.first_seen)<=days*86400000)&&(source==='all'||source==='linked'&&p.variants.length>1||p.source_types.includes(source)||p.variants.some(v=>'journal:'+v.journal_id===source))&&(!query||[p.title,p.abstract,...p.authors,p.reason,p.intro_zh,p.intro_en,...p.institutions,...p.tags,...p.journals].join(' ').toLowerCase().includes(query)));
