(async()=>{
 ...
 $('#journal-directory').innerHTML=Object.entries(d.journals.groups).map(([id,label])=>`<section class="source-group"><h3>${esc(label)}</h3><table><thead><tr><th>期刊</th><th>接入状态与累计收录</th></tr></thead><tbody>${d.journals.journals.filter(j=>j.enabled!==false&&j.group===id).map(j=>{const s=d.sources['journal:'+j.id],total=d.papers.filter(p=>p.variants.some(v=>v.journal_id===j.id)).length;return `<tr><td><a href="${esc(j.url)}" target="_blank" rel="noopener">${esc(j.name)} ↗</a><span class="source-state">ISSN ${esc(j.issn)}</span><a href="./?journal=${encodeURIComponent(j.id)}">本站已收录 ${total} 篇 →</a></td><td>${!s?'待首次抓取':s.status==='ok'?`接口正常 · 本次检索 ${s.fetched} 条 / 保留 ${s.retained} 条`:'抓取失败 · 保留历史数据'}${s?.fetched===0?' · 本次检索为空，不代表期刊没有新文章':''}<span class="source-state">最近成功 ${esc(when(s?.last_success))}${s?.coverage_since?' · 起始检索 '+esc(s.coverage_since):''}</span></td></tr>`}).join('')}</tbody></table></section>`).join('');}
 if(!$('#statistics'))return;
 const journals=d.journals.journals.filter(j=>j.enabled!==false),monthly=d.monthly_statistics||{},now=new Date(),months=[];
